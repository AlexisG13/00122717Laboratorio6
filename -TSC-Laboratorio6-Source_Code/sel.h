...
Matrix createLocalK(int element,mesh &m){
    ...
    E = m.getParameter(CONST_E);
    f = m.getParameter(CONST_f);
    Matrix K,A,B,Bt,At;
    ...
    productRealMatrix(-(E*a*Ae)/(2*pow(D,2)),productMatrixMatrix(Bt,productMatrixMatrix(At,productMatrixMatrix(A,B,2,2,3),2,2,3),3,2,3),K);
